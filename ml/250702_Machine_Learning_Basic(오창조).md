...
#### ✅ 5.시계열 데이터의 처리
- 1 시계열 데이터의 특성
  - 1.1 시계열 데이터와 순차적 데이터(순차적 데이터 > 시계열 데이터)
    - 시계열 데이터 : 시간 순서로 관측된 일련의 데이터. 일반적으로 동일한 시간간격마다 측정된 값들의 시퀀스
    - 순차적 데이터 : 어떠한 순서를 가지는 데이터로, 순서가 바뀌면 의미를 잃어 버림.
  - 1.2 시계열 데이터의 구성성분
    - 규칙(체계적)성분
      - 추세(Trend) 단기적으로 변화하더라도 장기적으로는 방향성을 보이는 것
      - 계절성(Seasonality) : 계절에 따른 변화처럼 일정하게, 정기적으로 데이터가 변화
      - 순환성(Cycle) : 데이터가 주기적으로 변화하기는 하지만, 일정하지 않은 주기로 변화하는 것
    - 불규칙 성분 : 랜덤한 원인에 의한 변동으로 예측이 불가능한 노이즈 성분.
  - 1.3 시계열 데이터의 특성
    - 기술적 분석 : 데이터의 기본 구조와 패턴을 이해하고, 이상치를 탐지하거나 데이터의 일반적인 행동을 설명함.
    - 예측적 분석 : 미래의 데이터를 예측하기 위해 과거 데이터의 분석
    - 시계열 예측 : 기본적으로 과거가 미래에 영향을 주기 때문에 시계열 데이터
    - 독립 항등 분포(IID; Independent and Identically Distributed) : 어떤 확률변수들이 같은 확률분포로부터 생성된 독립적인 샘플이라면 각 변수가 IID이라고 한다.
    - 자기 상관(Autocorrelation) : 한 시계열 내에서 자기자신과 다른 시점과의 상관관계 t시점과 시차 k이후인 t+k 시점 데이터의 상관계수로 측정
      - 시차 1의 자기 상관이 높으면 오늘의 데이터는 어제의 데이터가 가장 잘 설명한다는 의미
    - 마르코프 속성
    - 시계열의 정상성
    - 
- 2 시계열 데이터 모델링 방법론
